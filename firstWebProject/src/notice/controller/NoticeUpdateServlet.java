...
public class NoticeUpdateServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 파일 업로드 기능이 포함된 공지글 수정 처리용 컨트롤러
				...
				// 2. 업로드할 파일의 용량 설정 : 10Mbyte로 제한
				int maxSize = 1024 * 1024 * 10;
				// 3. 업로드할 파일의 저장 폴더 지정
				// 저장 폴더가 현재 구동중인 웹 컨테이너(WAS : 톰캣)에서 
				// 실행중인 웹 어플리케이션 안에 있는 폴더일 경우
				// 루트 경로 알아냄
				String root = request.getSession().getServletContext().getRealPath("/");
//				System.out.println("root : "+ root);
				// 업로드 되는 파일이 저장되는 폴더명과 루트를 연결함.
...
//				System.out.println("savePath : "+savePath);
				...
				// 객체 생성과 동시에 자동 파일이 업로드 된다. : 지정한 폴더에 자동 저장
				...
				// 자동 파일명숫자.확장자로 rename 한다.
				// 그래서 이런 문제를 막기위해 저장 폴더에 파일명을 바꾼다.
				// 주로 "년월일시분초.확장자"형식으로 rename 한다.
				...
				if(originalFileName != null) {
					...
					if(!originFile.renameTo(renameFile)) {
						...
						// 원본 파일을 읽기 위한 입력 스트림 생성
						FileInputStream fin = new FileInputStream(originFile);
						// 읽은 내용 기록할 복사본 파일에 대한 출력 스트림 생성
				...
				// 서비스 모델로 notice 객체 전달하고 결과받기
				int result = new NoticeService().updateNotice(notice);
				// 받은 결과를 가지고 성공/실패 뷰 선택 내보내기
				if(result > 0) {
					// 공지글 수정 성공시에는 공지글 상세 보기 출력

...
function showWriteForm(){
	location.href = "/testmb/views/board/boardWriteForm.jsp";
}
...
<div id="titleDiv">
	<form action="/testmb/bsearcht" method="post">	
	<label>검색할 제목을 입력하시오 : 
...
<div id="writerDiv">
	<form action="/testmb/bsearchw" method="post">	
...
<div id="dateDiv">
	<form action="/testmb/bsearchd" method="post">	
	<label>검색할 날짜를 선택하시오 : 
...
<tr><td align="center">${b.boardNum }</td>
	...
	<c:if test="${!empty loginUser }">
		<a href="/testmb/bdetail?bnum=${b.boardNum }&page=${ currentPage }">${b.boardTitle }</a>
	</c:if>
	<c:if test="${empty loginUser }">
		${b.boardTitle }
	</c:if>
...
<c:if test="${ currentPage gt 1 }">
	<a href="/testmb/blist?page=1">[맨처음]</a>&nbsp;
</c:if>
<c:if test="${ (currentPage - 10) < startPage && (currentPage - 10) > 1 }">
	<a href="/testmb/blist?page=${ startPage - 10 }">[prev]</a>
</c:if>
...
<c:forEach var="p" begin="${ startPage }" end="${ endPage }"> 
	...
	<c:if test="${ p ne currentPage }">
		...
		<a href="/testmb/bsearcht?keyword=${ keyword }&page=${ p }">${ p }</a>
		</c:if>
		<c:if test="${ !empty search and search eq 'writer' }">
		<a href="/testmb/bsearchw?keyword=${ keyword }&page=${ p }">${ p }</a>
		</c:if>
		<c:if test="${ !empty search and search eq 'date' }">
		<a href="/testmb/bsearchd?begin=${ begin }&end=${ end }&page=${ p }">${ p }</a>
		</c:if>
	<a href="/testmb/blist?page=${ p }">${ p }</a>
	</c:if>
</c:forEach>
<c:if test="${ (currentPage + 10) > endPage && (currentPage + 10) < maxPage }">
	<a href="/testmb/blist?page=${ endPage + 10 }">[next]</a>&nbsp;
</c:if>
<c:if test="${ (currentPage + 10) <= endPage || (currentPage + 10) >= maxPage }">
	[next]&nbsp;
</c:if>
...
<c:if test="${ currentPage lt maxPage }">
	<a href="/testmb/blist?page=${ maxPage }">[맨끝]</a>
</c:if>
</div>
<hr>
